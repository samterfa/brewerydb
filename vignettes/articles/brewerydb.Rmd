...
R has a very handy but dangerous function called sink() which will funnel all output that would normally go to the console to a file instead. If we're clever with it we can generate a .R file with functions on the fly. *Warning:* If you forget to turn the sink off so to speak, you won't get any output from functions you run later. You will think you've broken RStudio and start Googling for fixes until you realize that all the console output is still going to the .R file!!! Hypothetically speaking...
